...
* **Aim**: To move a stack of `n` disks from one rod to another following specific constraints (only one disk moves at a time and no larger disk sits on a smaller one).
...
* **Time Complexity**: **`O(log v)`** where `v` is the initial velocity (the velocity decreases geometrically).
* **Algorithm**:
1. **Base Case**: If `velocity <= 1`, return the current `count`.
2. **Recursive Step**: Multiply `velocity` by the restitution factor (0.575) and increment `count`.
...
* **Aim**: To calculate `a^b` efficiently using the "divide and conquer" approach.
...
* **Aim**: To evaluate a polynomial at a specific value `x` using the minimum number of multiplications.
...
* **Algorithm**:
1. **Base Case**: `if (n == poly.size() - 1) { return poly[n]; }`
...
* **Use Case**: Sorting small datasets and demonstrating quadratic (`O(n^2)`) time complexity.
...
* **Aim**: To find a single missing number in a range `1` to `n` without using arithmetic operators.
...
* **Algorithm**:
1. **XOR Range**: Recursively XOR all integers from `1` to `n`.
...
* **Time Complexity**: **`O(n^2)`** where `n` is the number of elements (due to nested recursion checking previous elements).
* **Algorithm**:
1. **Outer Recursion**: Traverses each element `i` of the array.
2. **Inner Recursion**: Checks all elements from index `0` to `i-1`.
